...
fun main() {
    val (N, M, K )=readlnInts()
    val arr=Array(N){ mutableListOf<IntArray>() }
    repeat(M){
        val x=readlnInts()
        arr[x[0]-1].add(intArrayOf(x[1]-1,x[2]))
    }
    val set=readlnInts()
}
private fun dijkstra(size: Int, adjacencyList: Array<List<IntArray>>, startingVertex: Int = 0) {
    val cost = LongArray(size) { Long.MAX_VALUE }
    ...
    val t = PriorityQueue<Pair<Long, Int>>(Comparator.comparing { it.first })
    val list = ArrayList<Int>()
    t.add(Pair(0L, startingVertex))
    while (!t.isEmpty()) {
        val a = t.poll().second
