...
fun main() {
    ...
    val f=Scanner(File("consistency_chapter_1_input.txt"))
    val o=File("outputA.txt").printWriter()
    ...
    repeat(t){
        ...
        for (c in 'A'..'Z'){
            var cost=0
            val bool=c in vowels
            for (cc in str){
                if(cc == c) continue
                if (cc in vowels){
                    cost+= if (bool) 2 else 1
                }else{
                    cost+= if (!bool) 2 else 1
                }
            }
        ...
        o.println("Case #${it+1}: $min")
        println("Case #${it+1}: $min")
    }
    o.close()
}
