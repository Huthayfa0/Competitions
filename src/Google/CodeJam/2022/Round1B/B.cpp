...
void solve(){
ll n,m;cin>>n>>m;ML arr(n,VL(m));
    lp(i,0,n)re(j,arr[i]);
    vector<pair<ll,ll>> as(n,mp(inf,0));
    lp(i,0,n){
        lp(j,0,m){
            chmin(as[i].F,arr[i][j]);
            chmax(as[i].S,arr[i][j]);
        }
    }
    vector<pair<ll,ll>> ans(n);
    ans[0].S=as[0].S;
    ans[0].F=as[0].S+(as[0].S-as[0].F);
    lp(i,1,n) {
        ans[i].F=min(ans[i-1].F+abs(as[i-1].F-as[i].S)+as[i].S-as[i].F,ans[i-1].S+abs(as[i-1].S-as[i].S)+as[i].S-as[i].F);
        ans[i].S=min(ans[i-1].F+abs(as[i-1].F-as[i].F)+as[i].S-as[i].F,ans[i-1].S+abs(as[i-1].S-as[i].F)+as[i].S-as[i].F);
    }
    cout<<min(ans[n-1].F,ans[n-1].S)<<endl;
}
