...
fun main() {
    val t=readlnInt()
    repeat(t) { cc ->
        val (d, n, k) = readlnInts()
        val queues = Array(d) { mutableListOf<Int>() }
        val que = Array(d) { mutableListOf<Int>() }
        val att = Array(n) { index ->
            readlnInts().also { queues[it[1]-1].add(index);que[it[2]-1].add(index) }[0]
        }
        var sum = 0L
        val answers = LongArray(d)
        val queue = PriorityQueue<Int>()
        val set = LinkedList<Int>()
        for (i in 0 until d) {
            queue.addAll(queues[i].map { att[it].also { x -> sum += x } })
            var s = sum
            while (queue.size > k) {
                set.add(queue.poll().also { x -> s -= x })
            }
            answers[i] = s
            queue.addAll(set)
            set.clear()
            que[i].map { att[it] }.forEach { sum -= it ;queue.remove(it) }
        }
        println("Case #${cc + 1}: ${answers.maxOrNull()!!}")
    }
}
