...
fun main() {
    ...
    repeat(t){
        ...
        /*val min=IntArray(5){ Int.MAX_VALUE}
        ...
        val x=index.groupBy { it }.filter { it.value.size>2 }
        if (!x.isEmpty())
            println( x.entries.first().key+1)
        else{
            var xx=-2
            for (i in 0 until n) {
                var a = 0
