...
fun main() {
    val modulas=998244353
    val n=readlnInt()
    val portals=readLongArray2d(n,3)
    val xn=LongArray(n)
    val yn=LongArray(n)
    val booleans=BooleanArray(n)
    for (i in portals.indices){
        xn[i]=portals[i][0]
        yn[i]=portals[i][1]
        booleans[i]=portals[i][2]==1L
    }
    var sum=0L
    var cur=0L
    while (cur<xn[n-1]){
        var a=BSfindFirst(0,n,cur){if (it>=n) Long.MAX_VALUE else xn[it]}
        while (a<n&&!booleans[a]){
            booleans[a++]=true
        }
        if (a==n){
            sum=(sum+(xn[n-1]-cur))%modulas
            break
        }else{
            sum=(sum+(xn[a]-cur))%modulas
            cur=yn[a]
            booleans[a]=false
        }
    }
    println((sum+1)%modulas)
