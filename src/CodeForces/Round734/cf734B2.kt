...
fun main() {
    ...
    repeat(t){
        ...
        arr.forEachIndexed { index, i -> map.putIfAbsent(i,HashSet())
        map[i]!!.add(index)}
        val ans=IntArray(n)
        //val sum=map.map { if (it.value.size>k) k else it.value.size}.sum()
        //val c=sum/k
        val f= mutableListOf<Int>()
        map.forEach { t, u ->f.addAll(u.take(k))}
        val c=f.size/k
        var x=1
        for (i in 0 until c*k){
            ans[f[i]]=x
            when(x){
                k->x=1
                else->x++
