...
fun main() {
    ...
    repeat(t){_->
        ...
        arr.forEach { it.sortDescending() }
        var bestCount=0
        arr.forEach {
            if (it[0]<=0)return@forEach
            var sum=it[0]
            var count=1
            for (i in 1 until n){
                sum+=it[i]
        ...
        /*val sum= List(5){ i->arr.sumBy {it[i]}}
        if (isIntresting(sum))
            println(n)
        else {
            var ans=false
            var x=n
            for (i in 1 until n) {
                //isIntresting(sum.mapIndexed { i,it->it-u.map {x-> adjacencyList[x][i] }.sum() })
                if (combinationsIteration(n, i) {isIntresting(sum.mapIndexed { i,a->a-it.map {x-> arr[x][i] }.sum() })}) {
                    ans = true
                    x=i
                    break
                }
        ...
        //println(n-bfs(n,arr,sum))*/
    }
}
fun isIntresting(x: List<Int>):Boolean{
    val sum=x.sum()
    for (a in x){
        if (2*a>sum)//a>sum-a
            return true
...
private fun bfs(size: Int, adjacencyList: Array<IntArray>,sum:IntArray):Int {

    val queue = LinkedList<HashSet<Int>>() // list used as a queue
    val isVisited = HashSet<HashSet<Int>>()
    val v=HashSet<Int>()
    queue.offer(v) // Enqueue v
    isVisited.add(v)
    while (!queue.isEmpty()) {
        val u = queue.poll() // Dequeue to u
        if (isIntresting(sum.mapIndexed { i,it->it-u.map {x-> adjacencyList[x][i] }.sum() }))
            return u.size
        for (i in 0 until size){
            if (!u.contains(i)){
                val x=u+i
                if (!isVisited.contains(x)){
                    queue.offer(x as HashSet<Int>)
                }
            }
        }
    }
...
//combinations
private fun combinationsIteration(n: Int, m: Int, f: (LinkedList<out Int>) -> Boolean):Boolean {
    val list = LinkedList<Int>()
    var count = 0
    var ans=false
