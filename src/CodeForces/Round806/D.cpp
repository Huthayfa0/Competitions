...
void solve(){
   ll n;cin>>n;
   vector<string> arr(n);re(i,arr);
   vector<string> savings=arr;
   sort(all(arr));
   arr.erase(unique(all(arr)),arr.end());
   vector<bool> ans(sz(arr),false);
   lp(i,0,sz(arr)){
       lp(j,1,sz(arr[i])){
           string x=arr[i].substr(0,j),y=arr[i].substr(j);
           auto it= lower_bound(all(arr),x);
           if(x==*it){
               it= lower_bound(all(arr),y);
               if(y==*it){
                   ans[i]=true;
                   break;
               }
   ...
   lp(i,0,n){
       auto it= lower_bound(all(arr),savings[i]);
       cout<<ans[it-arr.begin()];
   }
   cout<<endl;
}
