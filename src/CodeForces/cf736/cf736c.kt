...
fun main() {
    val (n,m)=readlnInts()
    val adjacency= List(n+1){ mutableSetOf<Int>() }
    var killed=0
    repeat(m){
        val (a,b)=readlnInts()
        if (adjacency[min(a,b)].isEmpty())
            killed++
        adjacency[min(a,b)].add(max(a,b))
        //adjacency[b].add(a)
    }
    fun cal():Int{
        var count=0
    ...
    repeat(t){
        val x=readlnInts()
        when(x[0]){
            1->{val (a,b)=Pair(x[1],x[2])
                if (adjacency[min(a,b)].isEmpty())
                    killed++
                adjacency[min(a,b)].add(max(a,b))}
            2->{val (a,b)=Pair(x[1],x[2])
                adjacency[min(a,b)].remove(max(a,b))
                if (adjacency[min(a,b)].isEmpty())
                    killed--}
            3-> println(n-killed)
        }
