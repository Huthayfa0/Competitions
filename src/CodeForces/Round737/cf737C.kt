...
fun main() {
    val t=readlnInt()
    val x=1000000007
    val compnations=getCombinationCountArray(2500,2500)
    fun comp(n:Int,m:Int):Int= when {
        m==0->1
        n==0->0
        n==m->1
        else->if (n <=2500&&m<=2500) compnations[n][m]else  (comp(n - 1, m - 1) + comp(n - 1, m)) % x
    }
    fun Int.to(exponent: Int): Int {
        if (exponent == 0) {
            return 0
        }
        if (exponent == 1) {
        ...
        return if (exponent%2==0) {
           ( (half.toLong() * half.toLong())%x).toInt()
        } else {
           ( (((half.toLong() * half.toLong())%x) * this.toLong())%x).toInt()
        }
    }
    repeat(t){
        val (n,k)=readlnInts()
...
private fun getCombinationCountArray(n:Int,m:Int):Array<IntArray>{
    require(n>=m)
    val x=1000000007
    ...
    for (i in 0 until m+1)
        for (j in 0 until n+1)
            arr[i][j]=when{
                j==0->1
                i==0->0
                else-> ((arr[i-1][j-1].toLong()+arr[i-1][j].toLong())%x).toInt()
            }
    return arr
}
