...
fun main() {
    ...
    repeat(t){
        ...
        val maps= str.groupBy { it }.mapValues { it.value.size }.toMutableMap()
        val x=maps.size
        val set= mutableSetOf<Char>()
        val chars=CharArray(x){'$'}
        var ii=x
        for (c in rev){
            if (set.add(c))
                chars[--ii]=c
        ...
        for (i in chars.indices){
            if (maps[chars[i]]!!%(i+1)!=0) {
                broke = true
                break
            }
            maps[chars[i]]=maps[chars[i]]!!/(i+1)
        }
        if (broke){
            println(-1)
            return@repeat
        ...
        if (broke) println(-1)
        else println("$ans ${chars.joinToString(separator = "")}")
    }
}
