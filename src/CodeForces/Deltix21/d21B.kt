...
fun main() {
    ...
    repeat(t){
        val n=readlnInt()
        val arr=readlnInts().map { it%2 }
        var count=arr.count { it==1 }
        if (count>(n+1)/2)
            println(-1)
        else{
            count=0
            ...
            //1 for wrong 1
            //0 for wrong 0
            val inda1= mutableListOf<Int>()//1 0 1
            val inda0= mutableListOf<Int>()
            val indb1= mutableListOf<Int>()//0 1 0
            val indb0= mutableListOf<Int>()
            for (i in 0 until n){
                if (i%2==0){
                    if (arr[i]==1){
                        indb1.add(i)
            ...
            if (inda0.size!=inda1.size&&indb0.size!=indb1.size)
                println(-1)
            else{
                if (n%2==0) {
                    var s1 = 0
                    for (i in inda1.indices) {
                        s1 += abs(inda1[i] - inda0[i])
                    }
                    var s2 = 0
                    for (i in indb1.indices) {
                        s2 += abs(indb1[i] - indb0[i])
                    }
                    println(min(s1, s2))
                }else{
                    if (inda0.size==inda1.size){
                        var s1 = 0
                        for (i in inda1.indices) {
                            s1 += abs(inda1[i] - inda0[i])
                        }
