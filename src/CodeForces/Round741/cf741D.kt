...
fun main() {
    ...
    repeat(t){_->
        val (n,q)=readlnInts()
        val arr=readln().mapIndexed { index, c -> if (c=='+'){
            if (index%2==1)-1 else 1
        }else{
            if (index%2==1)1 else -1
        } }//.map { if (it=='+')1 else -1 }
        val sum=IntArray(n)
        ...
        repeat(q){
            ...
            var ans=if (a==0)
                ...
                (abs(sum[b]-sum[a-1]))
            /*val t=LinkedList<List<Int>>()
            t.add(listOf(a,ans,0))
            ...
            while (t.isNotEmpty()){
                ...
                for (i in x[0]..b){
                    val l= listOf(i+1,ans-arr[i]-2*(sum[b]-sum[i]),x[2]+1)
                    if (!set.add(l))
                        continue
                    t.add(l)
                }*/
