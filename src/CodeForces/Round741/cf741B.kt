...
fun main() {
    ...
    val set=(1..9).filter { it.isPrime() }
    val set2=set.map {i-> set.map { i*10+it }.filter { it.isPrime() } }.reduce { acc, list -> acc+list }
  //  println(set2.joinToString())
   ...
   // println((373).isPrime())
    ...
    repeat(t){
        val n=readlnInt()
        val str=readln().also { require(it.length==n) }
        val groups=str.groupBy { it }
        val s=groups.keys.map { it-'0' }-set
        if (s.isNotEmpty()){
            println(1)
            println(s.first())
        }else{
